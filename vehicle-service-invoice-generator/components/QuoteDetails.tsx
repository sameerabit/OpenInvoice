...
interface QuoteDetailsProps {
  data: QuotationData;
  onDataChange?: (field: keyof QuotationData, value: string) => void;
}
...
const QuoteDetails: React.FC<QuoteDetailsProps> = ({ data, onDataChange }) => {
  return (
    <div className="flex justify-between mb-8 text-sm">
      ...
      {/* Right side - Quote & Vehicle Details */}
      <div className="w-1/2">
        <div className="grid grid-cols-[80px_1fr] gap-x-2 gap-y-1 ml-auto max-w-sm">
           <DetailLabel>Date:</DetailLabel>
           <EditableInput value={data.date} onChange={onDataChange ? (val) => onDataChange('date', val) : undefined} ariaLabel="Quote date" />
          
           <DetailLabel>Quote No:</DetailLabel>
           <EditableInput value={data.quotationNumber} onChange={onDataChange ? (val) => onDataChange('quotationNumber', val) : undefined} ariaLabel="Quotation number" />
