...
interface CreateSaleProps {
  ...
  onCreateInvoice: (payload: any) => void;
  onUpdateInvoice?: (id: string, payload: any) => void;
  initialData?: any; // InvoiceData or QuotationData
  isQuote?: boolean; // Reusing for Quote as well? User said "sales", but maybe useful.
}

const CreateSale: React.FC<CreateSaleProps> = ({ customers, services, products, onCreateInvoice, onUpdateInvoice, initialData }) => {
  ...
  useEffect(() => {
    if (initialData) {
      if (initialData.customerId) {
        ...
        // and pre-fill items? For now let's assume registered customers mostly.
        ...
        // Can't easily split address/phone if not structured
        }
      }
      ...
      const mappedItems: InvoiceItem[] = initialData.lineItems.map((item: any) => ({
        ...item,
        type: item.price === 0 && item.included ? 'product' : 'service', // Simple heuristic, might need better 'type' from backend if persisted
        // Actually backend line items don't store 'type' explicitly like 'service'|'product' in the same way 
        // but we can infer or default to service. The ID might match a service or product.
      }));
  ...
  const handleSubmit = async (actionStatus: 'DRAFT' | 'ISSUED') => {
    ...
    const payload = {
      ...
      status: actionStatus, // Pass status explicitely
    ...
    try {
      if (initialData && initialData.id && onUpdateInvoice) {
        await onUpdateInvoice(initialData.id, payload);
        alert(`Sale ${actionStatus === 'DRAFT' ? 'updated' : 'issued'} successfully`);
      } else {
        await onCreateInvoice(payload);
        alert(`Sale ${actionStatus === 'DRAFT' ? 'saved as draft' : 'created and issued'} successfully`);
      }
    } catch (error: any) {
  ...
  return (
    <div className="space-y-6 max-w-5xl"> {/* Left aligned container */}
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-slate-900">{initialData ? 'Edit Sale' : 'New Sale'}</h1>
          <p className="mt-1 text-slate-500">{initialData ? 'Update existing sale details.' : 'Create a new invoice for a customer.'}</p>
        </div>
      </div>
      ...
      <form onSubmit={(e) => e.preventDefault()} className="space-y-8">
        ...
        <div className="pt-4 border-t border-slate-200 flex justify-end gap-4">
          ...
          >
            {initialData && initialData.status === 'ISSUED' ? 'Update Invoice' : 'Issue Invoice'}
          </Button>
        </div>
      </form>
